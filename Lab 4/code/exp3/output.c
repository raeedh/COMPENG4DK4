...
void output_blip_to_screen(Simulation_Run_Ptr simulation_run) {
    ...
    if ((data->blip_counter >= BLIPRATE) || (data->number_of_packets_processed >= RUNLENGTH)) {
        ...
        percentagedone = 100 * (double) data->number_of_packets_processed / RUNLENGTH;
...
void output_results(Simulation_Run_Ptr this_simulation_run) {
    ...
    xmtted_fraction = (double) sim_data->number_of_packets_processed / sim_data->arrival_count;
    ...
    printf("Mean collisions per packet = %.3f\n", (double) sim_data->number_of_collisions / sim_data->number_of_packets_processed);

    for (i = 0; i < NUMBER_OF_STATIONS; i++) {

        printf("Station %2i Mean Delay = %8.1f \n", i,
               (sim_data->stations + i)->accumulated_delay / (sim_data->stations + i)->packet_count);
    }
    printf("\n\n");
}
