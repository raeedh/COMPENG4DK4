...
int main()
{
  ...
  {
    // printf("\nArrival rate = %f\n", arrival_rates[j]);
    int random_seeds[] = {400188200, 1882004, 18820040, 188200400, 882004001, 820040018, 200400188, 4001882, 40018820, 400190637, 1906374, 19063740, 190637400, 906374001, 63740019, 637400190, 374001906, 740019063};
    ...
    for (int i = 0; i < 18; i++)
    {
      ...
      service right away. */
        ...
        if (total_served % BLIP_RATE == 0)
          printf("Arrival rate = %f, Random seed = %d, Customers served = %ld (Total arrived = %ld)\r",
            ARRIVAL_RATE, RANDOM_SEED, total_served, total_arrived);
      }
    ...
    printf("%f, %f, %f, %f, %f                                                                                                                                \n", arrival_rates[j], util/18.0, fraction_served/18.0, mean_number_system/18.0, mean_delay/18.0);
    /* Halt the program before exiting. */
    // printf("Hit Enter to finish ... \n");
    // getchar(); 
